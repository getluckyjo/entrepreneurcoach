 ...
 *
 * Twitter's card validator prefers raster (PNG/JPG). LinkedIn, Slack, iMessage,
 * WhatsApp, and Facebook all accept SVG. For Twitter rasters, see seo/launch-checklist.md.
 */
...
/** Compute the OG image URL for a given pathname. Falls back to default. */
export function ogPathFor(pathname: string) {
  const slug = pathname.replace(/^\/+|\/+$/g, "");
  if (!slug) return "/og/home.svg";
  // Journal posts share the journal panel; per-post raster TODO documented in launch checklist.
  if (slug.startsWith("journal/")) return "/og/journal.svg";
  if (ogPanels[slug]) return `/og/${slug}.svg`;
